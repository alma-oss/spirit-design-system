...
import { deleteStaleOwnedPaths, readOwnedPaths, removeStagingDump, writeOwnedPaths } from './ownedPaths.js';
...
const DEFAULT_OUT = path.join(REPO_ROOT, 'apps/docsite/content');
...
async function runFromOptions(opts: {
  ...
  if (!dryRun) {
    ...
    await deleteStaleOwnedPaths(outDir, previousOwned, new Set(nextOwned));
    await removeStagingDump(outDir);
...
sade('supernova-extract', true)
  ...
  .option(
    '-o, --out',
    'Output directory (canonical pages; default: apps/docsite/content from repo root)',
  )
