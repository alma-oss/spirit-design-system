import { componentSlugToDirectoryName, urlToRelativeMarkdownPath } from '../buildOutputPath';
...
describe('urlToRelativeMarkdownPath', () => {
  it('maps component tabs to Component Tab Pages', () => {
    expect(urlToRelativeMarkdownPath(new URL('https://spirit.supernova-docs.io/latest/components/button/overview-oxxMcy7u'))).toEqual({
      relativePath: 'packages/web-react/src/components/Button/docs/overview.md',
      sourceSection: 'components',
    });
    expect(
      urlToRelativeMarkdownPath(
        new URL('https://spirit.supernova-docs.io/latest/components/unstable-combobox/design-iZBfrVzQ'),
      ),
    ).toEqual({
      relativePath: 'packages/web-react/src/components/UNSTABLE_Combobox/docs/design.md',
      sourceSection: 'components',
    });
  });
  ...
  it('writes Folder Landings as index.md', () => {
    ...
    expect(
      urlToRelativeMarkdownPath(new URL('https://spirit.supernova-docs.io/latest/design/global-tokens/spacing/overview-bGc59YZU'))
        .relativePath,
    ).toBe('docs/design/global-tokens/spacing/index.md');
    expect(
      urlToRelativeMarkdownPath(new URL('https://spirit.supernova-docs.io/latest/introduction/spirit-design-system-tGrwryFa'))
        .relativePath,
    ).toBe('docs/introduction/index.md');
    expect(
      urlToRelativeMarkdownPath(new URL('https://spirit.supernova-docs.io/latest/migrations/migrations-abcDEF12')).relativePath,
    ).toBe('docs/migrations/index.md');
  });
});

describe('componentSlugToDirectoryName', () => {
  it('maps stable and unstable URL slugs to existing component directory conventions', () => {
    expect(componentSlugToDirectoryName('file-upload')).toBe('FileUpload');
    expect(componentSlugToDirectoryName('text-field-base')).toBe('TextFieldBase');
    expect(componentSlugToDirectoryName('unstable-combobox')).toBe('UNSTABLE_Combobox');
  });
});
...
describe('titleForCanonicalPage', () => {
  it('uses the file slug when the extracted title is the parent folder', () => {
    expect(titleForCanonicalPage('docs/development/helpers/links.md', 'Helpers')).toBe('Links');
    expect(titleForCanonicalPage('docs/design/icons/library.md', 'Icons')).toBe('Library');
    expect(titleForCanonicalPage('docs/design/global-tokens/spacing/customisation.md', 'Spacing')).toBe('Customisation');
  });

  it('keeps component tab titles and Folder Landings', () => {
    expect(titleForCanonicalPage('packages/web-react/src/components/Button/docs/overview.md', 'Button')).toBe('Button');
    expect(titleForCanonicalPage('docs/design/global-tokens/spacing/index.md', 'Spacing')).toBe('Spacing');
    expect(titleForCanonicalPage('docs/development/utilities.md', 'Utilities')).toBe('Utilities');
  });
});
