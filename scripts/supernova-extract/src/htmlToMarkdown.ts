 ...
 */
/**
 * Supernova landing cards wrap a heading + description in one `<a>`. Turndown
 * turns that into a markdown link with blank lines in the label, which MDX
 * does not parse. Collapse those anchors to a single-line label before convert.
 */
function flattenBlockAnchors(root: HTMLElement, document: Document): void {
  for (const anchor of [...root.querySelectorAll('a[href]')]) {
    if (!anchor.querySelector('p, div, h1, h2, h3, h4, h5, h6, li, br, ul, ol')) {
      continue;
    }

    const heading = anchor.querySelector('h1, h2, h3, h4, h5, h6');
    const title = (heading?.textContent ?? anchor.textContent ?? '').replace(/\s+/gu, ' ').trim();
...
export function extractMarkdownFromHtml(html: string, pageUrl: string): PageExtractResult {
  ...
  flattenBlockAnchors(main, document);
