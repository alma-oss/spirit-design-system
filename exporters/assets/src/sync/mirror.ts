...
export const mirrorAssets = async (brand: string, out: string, assets: ExportedAsset[]): Promise<TargetSyncResult> => {
  ...
  const svgEntries = (await readdir(out, { withFileTypes: true })).filter((entry) =>
    entry.name.endsWith(SVG_EXTENSION),
  );
  const symlink = svgEntries.find((entry) => entry.isSymbolicLink());

  if (symlink) {
    throw new ConfigError(`Output directory contains an SVG symlink: ${path.join(out, symlink.name)}`);
  }

  const currentFiles = svgEntries.filter((entry) => entry.isFile()).map((entry) => entry.name);
