...
export const normalizeAndCopySvg = (srcDir: string, distDir: string, logger: Logger = consoleLogger): boolean => {
  try {
    ...
    if (svgs.length > 0) {
      ...
      logger.info(`Successfully normalized and copied ${svgs.length} SVG files`);
    ...
    logger.error(`Error normalizing SVGs: ${errorMessage}`);
  }
};

export const buildSvg = (srcDir: string, distDir: string, logger?: Logger): boolean =>
  normalizeAndCopySvg(srcDir, distDir, logger);
