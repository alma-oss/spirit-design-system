...
export const buildConstants = (srcDir: string, file: string): boolean => {
  try {
    ...
    if (svgs.length > 0) {
      ...
      console.log(`Successfully created ${file} with ${svgs.length} icons`);
    ...
    console.error(`Error building icon constants: ${errorMessage}`);
