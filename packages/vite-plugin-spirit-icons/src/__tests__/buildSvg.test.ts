...
describe('buildSvg', () => {
  ...
  describe('normalizeAndCopySvg', () => {
    ...
    it('should build sprite and write normalized SVGs', async () => {
      ...
      const result = normalizeAndCopySvg(srcDir, distDir);
    ...
    it('should do nothing when only sprite.svg present (no writes expected)', async () => {
      ...
      const result = normalizeAndCopySvg('/virtual/src', '/virtual/dist');
  ...
  describe('buildSvg', () => {
    ...
    it('should return true when SVG files are found and processed', () => {
      (readdirMock as jest.Mock).mockReturnValueOnce(['add.svg']);
      (readFileSyncMock as jest.Mock).mockReturnValue('<svg viewBox="0 0 24 24"><path fill="#000" /></svg>');

      const result = buildSvg('/virtual/src', '/virtual/dist');

      expect(result).toBe(true);
    });

    it('should return false when no SVG files are found', () => {
      (readdirMock as jest.Mock).mockReturnValueOnce([]);
