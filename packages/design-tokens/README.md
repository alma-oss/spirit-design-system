...
Only the primary surface radius is affected. In particular, the Avatar token applies only to square Avatars and the Card
token applies only to boxed Cards. Component names usually match their CSS classes. The following tokens use the public
component name instead of the class that owns the surface:
