...
export interface PanelAriaProps {
  /** Panel id, linking back to the trigger via aria-labelledby. */
  id: string;
  /** Id of the trigger that labels this panel. */
  'aria-labelledby': string;
  /** Whether the panel is currently hidden from assistive technology. */
  'aria-hidden': boolean;
  /** Native `hidden` attribute, set only during SSR to avoid a flash of unhidden content. */
  hidden?: boolean;
}
