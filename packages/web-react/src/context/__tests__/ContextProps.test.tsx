...
import { ContextPropsProvider, useContextProps } from '../ContextProps';
...
const createWrapper =
  ...
  ({ children }: { children: ReactNode }) =>
    values.reduceRight<ReactNode>((acc, value) => <ContextPropsProvider value={value}>{acc}</ContextPropsProvider>, children);
...
describe('UniversalProvider', () => {
  const ContextA = createContext<{ value?: string } | null>(null);
  const ContextB = createContext<{ value?: string } | null>(null);

  const useContextValues = () => ({ a: useContext(ContextA), b: useContext(ContextB) });

  it('should provide multiple context values from a single component', () => {
    ...
    expect(result.current).toEqual({ a: { value: 'a' }, b: { value: 'b' } });
  });

  it('should not let one provided context value leak into another', () => {
    const { result } = renderHook(() => useContextValues(), {
      wrapper: ({ children }) => (
        <UniversalProvider values={[[ContextA, { value: 'only-a' }]]}>{children}</UniversalProvider>
      ),
    });

    expect(result.current).toEqual({ a: { value: 'only-a' }, b: null });
  });

  it('should let a nested UniversalProvider fully replace an inherited value', () => {
    const { result } = renderHook(() => useContextValues(), {
      wrapper: ({ children }) => (
        <UniversalProvider values={[[ContextA, { value: 'outer' }]]}>
          <UniversalProvider values={[[ContextA, { value: 'inner' }]]}>{children}</UniversalProvider>
        </UniversalProvider>
      ),
    });

    expect(result.current).toEqual({ a: { value: 'inner' }, b: null });
  });
});
