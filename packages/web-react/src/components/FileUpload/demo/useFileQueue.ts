...
import { type FileItemMetadata } from '../../File/types';
...
export interface UnstableFileUploadHandlingProps {
  addToQueue: (key: string, file: File, meta?: FileItemMetadata) => void;
  clearQueue: () => void;
  fileQueue: FileQueueMapType;
  findInQueue: (key: string) => FileQueueValueType | null;
  onDismiss: (key: string) => void;
  updateQueue: (key: string, file: File, meta?: FileItemMetadata) => void;
}

export interface FileQueueReturn extends UnstableFileUploadHandlingProps {}

function toDisplayValue(file: File, meta?: FileItemMetadata): FileQueueValueType {
...
export const useFileQueue = (): FileQueueReturn => {
  ...
  const addToQueueHandler = (key: string, file: File, meta?: FileItemMetadata) => {
    setQueue((prev) => new Map(prev.set(key, toDisplayValue(file, meta))));
  };

  const findInQueueHandler = (key: string) => queue.get(key) || null;

  const updateQueueHandler = (key: string, file: File, meta?: FileItemMetadata) => {
    setQueue((prev) => {
      const newState = new Map(prev);
