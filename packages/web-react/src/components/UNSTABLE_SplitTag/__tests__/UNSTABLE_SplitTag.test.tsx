...
describe('UNSTABLE_SplitTag', () => {
  ...
  it('should allow nested Tags and ControlButtons to override non-size split tag defaults', () => {
    render(
      <UNSTABLE_SplitTag color="selected" isDisabled isSubtle size="large">
        <Tag color="neutral" isDisabled={false} isSubtle={false}>
          Prague
        </Tag>
        <Tag elementType="button" aria-label="Remove Prague" isDisabled={false}>
          <ControlButton elementType="span" aria-hidden="true" isDisabled={false} isSymmetrical />
        </Tag>
      </UNSTABLE_SplitTag>,
    );

    expect(screen.getByText('Prague')).toHaveClass('Tag--neutral', 'Tag--large');
