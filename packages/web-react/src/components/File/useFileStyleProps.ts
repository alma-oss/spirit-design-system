...
import { type FilePreviewStyleProps } from './types';
import { type FileImageCropStyles, type FileImageObjectFitStyles, useFileImageStyles } from './useFileImageStyles';

export interface FileStyleProps extends StyleProps, Validation, FilePreviewStyleProps {
  isDisabled?: boolean;
}
...
export const useFileStyleProps = (props?: FileStyleProps): FileStyle => {
  const fileClass = useClassNamePrefix('File');
