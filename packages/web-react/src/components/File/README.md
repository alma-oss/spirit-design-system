...
`File` is a visual row for a single uploaded file. It is designed to compose with [`FileUpload`][file-upload-readme].
...
<Stack aria-label="Uploaded files" elementType="ul" hasSpacing>
  <File id="file-1" label="Document.pdf" helperText="2.5 MB" onDismiss={() => onDismiss('file-1')} />
</Stack>
```
...
Pass `previewSlot` with [`FileImagePreview`](#fileimagepreview-props) for a thumbnail:
...
<File
  ...
  previewSlot={<FileImagePreview imagePreview={previewUrl} label={`Preview of ${fileName}`} />}
/>
```
...
<File
  ...
  onDismiss={() => onDismiss('file-1')}
  previewSlot={
    <FileImagePreview imagePreview={imageUrl} label="Square crop preview" meta={cropMeta} imageObjectFit="cover" />
  }
/>;
```
...
set `strings.ariaLabel.remove` if the row can cancel the upload:
...
| `removeText`        | `string`                                       | —       | ✕        | _Deprecated, use `strings.ariaLabel.remove`_                                                                                    |
...
+ <File label="report.pdf" strings={{ ariaLabel: { edit: 'Edit', remove: { key: 'attachment.remove' } } }} />
```
...
| `meta`           | `FileItemMetadata`       | —       | ✕        | Optional crop metadata (`x`, `y`, `cropWidth`, `cropHeight`, `originalWidth`, `originalHeight`) |
