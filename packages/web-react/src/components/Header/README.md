# Header

The `Header` is a composition of several subcomponents:

- [Header](#header)
- [HeaderLogo](#headerlogo)

## Header

The `Header` component is a main wrapper which provides mainly the visual for the Header.

```tsx
import { Header } from '@alma-oss/spirit-web-react';

<Header>{/* Content go here */}</Header>;
```

You can set `hasBottomDivider` prop to add a bottom divider to the Header.

```tsx
import { Header } from '@alma-oss/spirit-web-react';

<Header hasBottomDivider>{/* Content go here */}</Header>;
```

It also sets a CSS custom property `--spirit-header-height` for the Header height,
which is used by nested components — notably `HeaderLogo` — to align their height.
See the [web package README][readme-header-web] for the full token definition.

### API

| Name               | Type                    | Default | Required | Description                              |
| ------------------ | ----------------------- | ------- | -------- | ---------------------------------------- |
| `children`         | `string` \| `ReactNode` | `null`  | ✓        | Content of the Header                    |
| `hasBottomDivider` | `bool`                  | `false` | ✕        | If true, the Header has a bottom divider |

The components accept [additional attributes][readme-additional-attributes].
If you need more control over the styling of a component, you can use [style props][readme-style-props]
and [escape hatches][readme-escape-hatches].

## HeaderLogo

`HeaderLogo` exists to inherit the `--spirit-header-height` CSS custom property set by `Header`
and apply it to the logo wrapper, ensuring the logo area matches the header height.

The `HeaderLogo` component is a container for the logo.

```tsx
import { HeaderLogo } from '@alma-oss/spirit-web-react';

<HeaderLogo>{/* Content go here */}</HeaderLogo>;
```

It inherits the `Header` height and sets the logo wrapper height to the same value.

You can use the `ProductLogo` component inside the `HeaderLogo` component.

```tsx
<HeaderLogo href="#" aria-label="JobBoard homepage">
  <ProductLogo>{/* Logo go here */}</ProductLogo>
</HeaderLogo>
```

👉 If the `HeaderLogo` is rendered as an anchor element,
make sure the link has an accessible name so screen reader users know where it leads.

This can be achieved by:

- Adding an `alt` attribute to an `<img>` tag inside (e.g., `<img alt="JobBoard homepage" />`)
- Including a `<title>` element in an SVG (e.g., `<svg><title>JobBoard homepage</title>...</svg>`)
- Adding an `aria-label` attribute to the link itself for decorative images

### API

| Name          | Type                    | Default | Required | Description               |
| ------------- | ----------------------- | ------- | -------- | ------------------------- |
| `children`    | `string` \| `ReactNode` | `null`  | ✓        | Content of the HeaderLogo |
| `elementType` | `ElementType`           | `a`     | ✕        | Type of element used as   |

## Component Composition

Use [`Container`][web-react-container] and [`Flex`][web-react-flex] components to create a layout for the Header content.

```tsx
<Header>
  <Container>
    <Flex alignmentX="left" alignmentY="center">
      <HeaderLogo href="#" aria-label="JobBoard homepage">
        <ProductLogo>{/* Logo go here */}</ProductLogo>
      </HeaderLogo>
      {/* Navigation go here */}
      {/* Other Navigation go here */}
    </Flex>
  </Container>
</Header>
```

This way you can easily modify the layout of the Header content.

For example you can make the content centered by setting the `Flex` alignment properties to center.

```tsx
<Header>
  <Flex alignmentX="center" alignmentY="center">
    <HeaderLogo href="#" aria-label="JobBoard homepage">
      <ProductLogo>{/* Content go here */}</ProductLogo>
    </HeaderLogo>
  </Flex>
</Header>
```

Or you can modify the gaps between the content by setting the `Flex` `spacing` property.

```tsx
<Header>
  <Container>
    <Flex alignmentX="left" alignmentY="center" spacing="space-500">
      <HeaderLogo href="#" aria-label="JobBoard homepage">
        <ProductLogo>{/* Logo go here */}</ProductLogo>
      </HeaderLogo>
      {/* Navigation go here */}
      {/* Other Navigation go here */}
    </Flex>
  </Container>
</Header>
```

If you need the whole Header fluid you can do it by adding the `isFluid` prop to the `Container`.

```tsx
<Header>
  <Container isFluid>
    <Flex alignmentX="left" alignmentY="center" spacing="space-500">
      <HeaderLogo href="#" aria-label="JobBoard homepage">
        <ProductLogo>{/* Content go here */}</ProductLogo>
      </HeaderLogo>
    </Flex>
  </Container>
</Header>
```

## With Navigation

You can use the [`Navigation`][web-react-navigation] component inside the `Header` component.

The `NavigationAction` components will inherit the `Header` height and set the navigation
action height to the same value.

Use the composition mentioned above to create the layout you need.

```tsx
<Header>
  <Container>
    <Flex alignmentX="left" alignmentY="center" spacing="space-1000">
      <HeaderLogo href="/" aria-label="JobBoard homepage">
        <ProductLogo>{/* Logo go here */}</ProductLogo>
      </HeaderLogo>
      <Navigation>
        <NavigationItem>
          <NavigationAction href="#">Link</NavigationAction>
        </NavigationItem>
        <NavigationItem>
          <NavigationAction href="#" aria-current="page" isSelected>
            Selected
          </NavigationAction>
        </NavigationItem>
        <NavigationItem>
          <NavigationAction href="#" isDisabled>
            Disabled
          </NavigationAction>
        </NavigationItem>
      </Navigation>
      <Navigation marginLeft="auto">
        <NavigationItem>
          <ButtonLink href="#" color="secondary">
            Sign up
          </ButtonLink>
        </NavigationItem>
        <NavigationItem>
          <ButtonLink href="#">Post a job</ButtonLink>
        </NavigationItem>
      </Navigation>
    </Flex>
  </Container>
</Header>
```

[readme-additional-attributes]: https://github.com/alma-oss/spirit-design-system/blob/main/packages/web-react/README.md#additional-attributes
[readme-escape-hatches]: https://github.com/alma-oss/spirit-design-system/blob/main/packages/web-react/README.md#escape-hatches
[readme-header-web]: https://github.com/alma-oss/spirit-design-system/blob/main/packages/web/src/scss/components/Header/README.md
[readme-style-props]: https://github.com/alma-oss/spirit-design-system/blob/main/packages/web-react/README.md#style-props
[web-react-container]: https://github.com/alma-oss/spirit-design-system/blob/main/packages/web-react/src/components/Container/README.md
[web-react-flex]: https://github.com/alma-oss/spirit-design-system/blob/main/packages/web-react/src/components/Flex/README.md
[web-react-navigation]: https://github.com/alma-oss/spirit-design-system/blob/main/packages/web-react/src/components/Navigation/README.md
