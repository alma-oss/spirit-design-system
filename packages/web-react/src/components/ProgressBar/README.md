...
React throttles how often `value` and `valueLabel` are committed (to the same interval as the CSS transition)
so rapid updates can finish animating instead of restarting on every frame.
...
ℹ️ Native `<progress>` already announces the numeric value. Set `aria-valuetext` when the visible
`valueLabel` should be announced instead (for example "4 out of 20 awards"). ProgressBar then hides the
visible `valueLabel` from assistive technologies so the value is not announced twice.
...
<ul aria-label="Uploaded files">
  <File label="Document.pdf" onDismiss={onDismiss} strings={{ ariaLabel: { remove: 'Cancel upload of Document.pdf' } }}>
