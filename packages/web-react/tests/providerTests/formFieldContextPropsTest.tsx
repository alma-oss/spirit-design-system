...
import { FormFieldTypes } from '../../src/types';
...
interface FormFieldHelperTextContextPropsTestConfig extends BaseFormFieldTestConfig, TypeContextProps {
  /** When false, skip inline class test (parents without `formFieldType` in context). @default false */
...
export const formFieldContextPropsTest = ({
  ...
  describe('prop priority (1. direct props, 2. context, 3. defaultProps)', () => {
    if (includeInlineType || includeItemType) {
      it('should use default formFieldType when no context and no direct prop', () => {
        renderFieldComponent(renderComponent);
        const element = screen.getByText(text);

        expect(element.className).toMatch(new RegExp(`^${classNamePrefix}\\b`));

        if (includeInlineType) {
    ...
    if (includeInlineType) {
      it('should use context formFieldType when context provides it and no direct prop', () => {
        render(<PropsProvider value={{ formFieldType: FormFieldTypes.INLINE }}>{renderComponent()}</PropsProvider>);
        const element = screen.getByText(text);
    ...
    if (includeInlineType && includeItemType) {
      it('should use direct formFieldType over context (direct props override context)', () => {
        render(
          <PropsProvider value={{ formFieldType: FormFieldTypes.ITEM }}>
            {renderComponent({ formFieldType: FormFieldTypes.INLINE })}
          </PropsProvider>,
        );
        const element = screen.getByText(text);
