...
If you need to indicate the state of the toggle, you can add the `Toggle--indicators`
modifier class to the input. This will add visual indicators to the toggle switch.
...
(`has-success`, `has-warning`, `has-danger`) on the input element and
...
The input position can be set using [Flex][readme-flex] direction and horizontal alignment modifiers.
...
>
  ...
  <div>
    <label class="Label Label--inline" for="toggle-position-end">Toggle Label</label>
  </div>
</div>
```
