...
2. Add closing button using `ControlButton ControlButton--medium text-color-scheme dynamic-color-background-interactive accessibility-tap-target ControlButton--hasBackground dynamic-color-border ControlButton--symmetrical` classes.
...
By default, both `click` and `hover` options are active, for example, `data-spirit-trigger="click, hover"`.
