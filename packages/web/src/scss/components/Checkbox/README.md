...
- Use `<button>` elements with link styling (e.g., `link-underlined`, `link-inherit`), NOT `<a>` tags, for modal
  triggers
- Use `aria-details` to link the checkbox to the details section
- Use `aria-describedby` to link helper text and validation text to the checkbox
- The `aria-details` attribute is for supplementary information, while `aria-describedby` is for auxiliary descriptions
