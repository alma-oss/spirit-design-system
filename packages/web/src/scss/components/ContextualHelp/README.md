...
<div class="Flex Flex--horizontal Flex--alignmentYCenter" style="--flex-spacing-x: var(--spirit-space-100);">
  <label class="Label Label--medium" for="languages">Languages</label>
...
`ControlButton--symmetrical` for the icon-only square. The `d-flex` utility centers the icon and keeps the
accessibility-hidden label from affecting the button layout.
...
Set `aria-describedby` on the trigger to the unique `id` of the TooltipPopover. This relationship lets assistive
technologies announce the supplemental tooltip content for the trigger.

⚠️ Do not use a non-focusable icon as the trigger.

## Composition

| Piece            | What to use                                                                                           | Default                       |
| ---------------- | ----------------------------------------------------------------------------------------------------- | ----------------------------- |
| Wrapper          | `.Tooltip` with `data-spirit-element="tooltip"`                                                       | —                             |
