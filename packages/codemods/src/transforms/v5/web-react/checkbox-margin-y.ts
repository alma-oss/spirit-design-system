...
const CHECKBOX_COMPONENT = 'Checkbox';
...
const transform = (fileInfo: FileInfo, api: API) => {
  ...
  const checkboxNamedImports = new Set<string>();
    ...
    .forEach((path) => {
      path.node.specifiers?.forEach((specifier) => {
        if (specifier.type === 'ImportSpecifier' && specifier.imported.type === 'Identifier') {
          if (specifier.imported.name === CHECKBOX_COMPONENT) {
            checkboxNamedImports.add(specifier.local?.name ?? specifier.imported.name);
          }
  ...
  if (checkboxNamedImports.size === 0 && namespaceImports.size === 0) {
    return fileInfo.source;
  }

  root.find(j.JSXOpeningElement).forEach((path) => {
    if (
      !isImportedElement(path.node, CHECKBOX_COMPONENT, checkboxNamedImports, namespaceImports) ||
