...
- A named `<form>` is exposed as a `form` landmark — give it an accessible name via
  `aria-labelledby` pointing to the nearest heading, or `aria-label` when no visible heading is
  present, so the form is reachable by landmark navigation.
...
- Password fields must carry `autocomplete="current-password"` (login) or
  `autocomplete="new-password"` (registration/change) so password managers can fill and save
  credentials. Pair `new-password` fields with a username field that has
  `autocomplete="username"` so managers associate the pair correctly.
