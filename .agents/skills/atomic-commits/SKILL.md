...
- **Body explains _why_, not _what_.** The diff already shows what changed. Use a `  -` dashed
...
Cluster the hunks by logical change, similar to branch naming types — e.g. build, chore, feat, … or unrelated changes that happened to be in the tree. Inspect anything ambiguous:
