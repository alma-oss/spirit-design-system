...
Prefer a Spirit component when it owns the needed semantics, behavior, or styling. Decide in this
order:

1. **Figma identity** — layer / component name and Code Connect (strongest hint).
2. **Layout pattern** — how items repeat, align, overflow, or compare; use the table below.
3. **Source search** — `packages/web-react/src/components` for a composite that already covers it.

A composite keeps both the look and the behaviour (overflow controls, feature tooltips, dividers).
...
| Pricing table / plan comparison          | `PricingPlan` + `Matrix`                              | Feature details use PricingPlan’s Tooltip/Modal composition                                                                  |
| Overflow region                          | `ScrollView`                                          | Wraps any overflowing content. Set `direction` to `horizontal` or `vertical` (default). Size children with layout components |
