...
| `*WHO:*` (literal single-asterisk wiki bold)         | `_WHO:_` (italic)                                 | The write path runs input through a Markdown-aware step first. Write Markdown `**WHO:**` instead — it converts correctly to wiki `*WHO:*`. Likewise use backtick `` `code` `` rather than literal `{{code}}` for monospace; it converts correctly to `{{code}}`. |
